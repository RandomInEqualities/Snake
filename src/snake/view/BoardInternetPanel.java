...
import snake.control.BoardInternetListener;
 ...
 */
public class BoardInternetPanel extends BoardBasePanel  implements Observer {
	...
	private BoardInternetListener control;
	
	public BoardInternetPanel(ViewFrame view) {
		super();
		this.game = null;
		this.snakeColorPlayerOne = ResourceColors.GREEN;
		this.snakeColorPlayerTwo = ResourceColors.RED;
		...
		control = new BoardInternetListener(view);
	...
	public void registerGame(GameMultiplayer newGame, Player player, String host) throws IOException {
		control.registerGame(newGame, player, host);
		if (game != null) {
	...
	public void closeConnection() {
		try {
			...
			System.out.println("unable to close connection: " + error.getMessage());
		}
	}
